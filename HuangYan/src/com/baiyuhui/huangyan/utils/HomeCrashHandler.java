	...
	public class HomeCrashHandler implements UncaughtExceptionHandler {
	    ...
	    private boolean handleException(Throwable ex) {
	        ...
	        new Thread() {
	            ...
	            public void run() {
	                Looper.prepare();
	                Toast.makeText(mContext, " 我们出错了,会尽快修改! ", Toast.LENGTH_SHORT).show(); 
	                Looper.loop();
	            }
	        }.start();
	    ...
	    private String saveCrashInfo2File(Throwable ex) {
	        ...
	        while (cause != null) {
	        	Log.i("HuangYan", "cause:"+cause.toString()+"--");
	            cause.printStackTrace(printWriter);
	            cause = cause.getCause();
	        }
	        printWriter.close();
	        String result = writer.toString();
	        Log.i("HuangYan", "result:"+result);
	        sb.append(result);
	        try {
	            ...
	            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
	                String path = "/sdcard/HuangYan/crash/";
