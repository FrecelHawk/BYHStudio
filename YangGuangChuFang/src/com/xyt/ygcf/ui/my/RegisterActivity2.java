...
/**
 * 
 * @author yuyangming 注册（可用手机注册，先保留）
 * 
 */
public class RegisterActivity2 extends BaseActivity {
	...
	private EditText etVerfify, etNickName, etPassword, etPassword2;
	...
	private void initViewData() {
		setTitle("注册");
		final EditText etPhone = (EditText) findViewById(R.id.activity_register_phone);
		final TextView vertifyView = (TextView) findViewById(R.id.activity_register_verify_btn);
	...
	private void requestToRegisterAcount() {
		if (!isRequesting[REQUEST_COMMIT]) {
			...
			requestRegister(verifyCode, password_1);
		}
	}

	private void requestRegister(String verifyCode, String password) {
		try {
			final RequestParams params = new RequestParams();
			password = HashAlgorithm.getHashText(password);
			params.addQueryStringParameter("memberNo", etNickName.getText().toString().trim());
			params.addQueryStringParameter("password", password);
			params.addQueryStringParameter("verificationCode", verifyCode);
	...
	public void handleJson(String json, int which) {
		SpHelper.init(this).setUserCode(account_1);
